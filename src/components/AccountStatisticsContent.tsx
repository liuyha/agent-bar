...
import { accountPeriodLabels } from '../lib/statisticsPeriods';
...
function AccountPeriodStatistics({ statistics, selectedPeriod, dateRange }: { statistics: AccountUsageSnapshot | null; selectedPeriod: TokenPeriod['period']; dateRange: StatisticsDateRange }) {
  ...
  return <section className="statistics-period account-period-statistics" aria-label={`${accountPeriodLabels[selectedPeriod]}服务端 Token 统计`}>
    <div className="statistics-period-heading"><h3>{accountPeriodLabels[selectedPeriod]}</h3><span>{filtered ? dateRangeLabel(dateRange) : all ? '覆盖范围以服务端为准' : !period ? '—' : period.startDate === period.endDate ? period.endDate : `${period.startDate} – ${period.endDate}`}</span></div>
