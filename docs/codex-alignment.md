...
- 额度采用默认账号采集链路。使用统计提供“本机记录”和“服务端”两个来源，服务端内部自动选择 PAT / OAuth / 受控 CLI 路径，支持账号 Token 活动与可选网页 Credits／代码审查补充，见 [账号服务端使用统计](account-statistics.md)；账号切换和工作区管理仍由原工具完成。
