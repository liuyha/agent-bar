...
模型单价核验日期：2026-09-16。下列模型价格表的单位均为 **USD / 1,000,000 tokens**，界面约等金额以人民币展示。

统计金额按本机会话日志中的模型与 token 分类，折算为公开 **Standard API 价格等值**。它不代表 Codex / Claude 订阅账单、实际付款、剩余额度或节省金额。使用核验日价目表重新估算历史用量，不还原历史合同或促销账单。

Rust 按美元单价计算并保存 `estimatedCostUsd`，前端展示时统一换算一次人民币。采用固定估算汇率 **1 USD ≈ 7 CNY**；这是为粗略估算选定的值，不是实时行情，也不按历史每日汇率重算。界面标注“约等金额 · 人民币”并显示固定汇率说明，使用 `¥` 和两位小数，非零且不足人民币 0.01 元时显示 `< ¥0.01`。人民币金额同样不代表实际付款或结算金额。
...
- 金额采用标准 API 等值，并按上述固定估算汇率换算人民币；不包含 Fast / Priority、Batch / Flex、地区附加费、工具单独计收的费用、税费、合同优惠、换汇手续费或订阅折扣。这些费率与订阅实际消耗的映射不能从本机日志完整恢复。
