...
以下记录按当时版本保留为历史。2026-09-17 已移除使用统计网页补充功能，早期记录中的网页开关、连接窗口、Credits／代码审查指标和网页脚本测试不代表当前功能或测试配置；当前使用统计范围见 [账号服务端使用统计](account-statistics.md)。
...
## 移除使用统计网页补充（2026-09-17）

- 删除网页开关、连接入口、独立登录窗口、网页采集脚本与相关数据类型、缓存合并逻辑及专用测试命令。使用统计继续提供本机记录和服务端来源，保留账号 ID、时段选择和刷新行为。
- 旧设置中的 `codexWebExtras` 和旧账号缓存中的 `web` 字段兼容读入后丢弃，后续保存不再写入；回归验证旧数据仍可恢复，来源、登录范围及并发请求保护继续生效。
- `pnpm check` 通过：TypeScript、ESLint、196 项前端测试和生产构建。Rustfmt 与 Clippy（`-D warnings`）通过；161 项 Rust 测试在沙盒内通过，3 项 HTTP mock 因回环监听限制失败，放行后单独执行 `cargo test --manifest-path src-tauri/Cargo.toml --locked --lib http_` 全部通过，合计 164 项离线测试通过。
- 4 项真实账号／本机读取测试保持默认忽略。本轮未执行真实账号接口、原生窗口交互或桌面应用打包验收。

## 费用预估人民币展示（2026-09-17）

- Codex、Claude 本机统计的五个时段统一显示人民币金额（`¥`），按固定估算汇率 `1 USD ≈ 7 CNY` 换算，并在页面注明；该值为粗略估算约定，不是实时行情。模型单价和缓存继续使用美元，展示时换算一次。
- 换算后保留两位小数，非零但不足人民币 0.01 元显示 `< ¥0.01`；零金额、无法估算和部分计价继续区分。
- `pnpm check` 通过：TypeScript、ESLint、201 项前端测试及生产构建。覆盖实际金额换算、分币阈值、非法输入、全部五个时段和部分计价。本轮未修改 Rust，也未执行原生交互或桌面应用打包验收。
