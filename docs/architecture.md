...
会话只在 Rust 中解析，不把对话正文或原始日志发送到前端。模型金额采用核验的公开标准美元单价，无价格的模型保留 Token 统计；`estimatedCostUsd` 汇总可计价部分，并通过 `unpricedTokens` 提示不完整，全部无法计价时为 null。美元基准保留在 Rust、接口及缓存中，前端展示时按固定估算汇率 `1 USD ≈ 7 CNY` 统一换算一次人民币，无需迁移已有缓存。该汇率是选定的粗略估算值，不是实时或历史逐日行情。界面显示“约等金额 · 人民币”和固定汇率说明，以 `¥` 和两位小数展示；非零且不足人民币 0.01 元显示 `< ¥0.01`，缺失金额与真实零值继续区分。费率来源与边界记录在 [计价依据](token-pricing.md)。本地数据可能跨账号，不能与当前账号额度比例混为一谈。
