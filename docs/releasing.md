...
[Release 工作流](../.github/workflows/release.yml) 仅在推送 `v*` 版本标签时触发，在 GitHub 托管的 macOS、Windows 和 Linux 运行器上打包，无需本机拥有这些系统。分支推送和 Pull Request 不触发工作流，也不提供手动运行入口。全部构建成功后，工作流统一创建或更新同标签的 **Release 草稿**，上传安装包和 `SHA256SUMS.txt`；维护者检查后再公开发布。
...
3. 按下方步骤推送版本标签后，在 [Actions](https://github.com/liuyha/agent-bar/actions) 中查看 **Release** 工作流。

Release 工作流会校验版本、执行前端检查及六种目标的 Rust 检查与测试，再构建安装包。
...
发布标签必须指向包含本次工作流的提交；重跑已有标签的运行仍使用该标签对应提交的工作流与代码，不会自动使用 `main` 上的新配置。已公开的版本应保留原标签，后续修改使用新版本标签。
...
- **临时网络或运行器失败**：打开 Actions 中该标签的运行记录，选择 **Re-run jobs → Re-run all jobs**。对未公开的同标签草稿，成功后的汇总步骤会更新安装包与校验文件。
